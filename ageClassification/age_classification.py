...
ageGroup_labels = adata.obs["ageGroup_id_labels"].tolist()  # make sure count from 0
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, schedule_interval= 1, gamma=0.9)
...
def test(model: nn.Module, adata: DataLoader):
    ...
    ageGroup_labels = adata.obs["ageGroup_id_labels"].tolist()
    ageGroup_labels = np.array(ageGroup_labels)
