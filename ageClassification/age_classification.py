...
#         """
#         :param gamma: Focusing parameter γ. Default is 2.
#         :param alpha: Weighting factor α in balanced cross-entropy. Default is None.
...
#         # 如果 alpha 被设置，确保它在与 targets 相同的设备上
...
samples = torch.tensor([16542, 12021, 8493, 247, 1], dtype=torch.float)
# total_samples = torch.sum(samples)
# alpha = total_samples / (samples * len(samples))
# print("Alpha values: ", alpha)

# criterion_cls = FocalLoss(gamma=2, alpha=alpha)

# weights = torch.tensor([1/1, 1/7641, 1/10790, 1/14914, 1/227], dtype=torch.float)
