...
parser = argparse.ArgumentParser(description="script with parameter.")
parser.add_argument('--epochs', type=int, default=20, help='Number of training epochs')
parser.add_argument('--lr', type=float, default=0.0001, help='Learning rate for the optimizer')
parser.add_argument('--batch_size', type=int, default=32, help='Batch size for training')
parser.add_argument('--dropout', type=float, default=0.2, help='Dropout for training')
parser.add_argument('--freeze', default=False, help='Freeze for training or not')
...
adata.obs["individual_index"] = adata.obs.index
adata_test.obs["individual_index"] = adata_test.obs.index
...
celltype_id_labels = adata.obs["sex"].astype("category").cat.codes.values
celltypes = adata.obs["sex"].unique()
...
print('freeze prcess =', config.freeze)
...
def evaluate(model: nn.Module, loader: DataLoader, return_raw: bool = False) -> float:
    ...
    return total_loss / total_num, total_error / total_num, total_truth/total_num
...
for epoch in range(1, epochs + 1):
    ...
    logger.info(
        ...
        f"valid loss/mse {val_loss:5.4f} | err {val_err:5.4f} | acc {acc:5.4f}"
    )
    logger.info("-" * 89)
